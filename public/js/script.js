...
const API_BASE_URL = 'https://5248-157-15-46-172.ngrok-free.app';
...
async function sendTelegramNotification(message, device = ACTIVE_DEVICE) {
  try {
    const res = await fetch(`${API_BASE_URL}/api/telegram-notify`, {
      method: 'POST',
      headers: { 
...
async function checkPauseStatus() {
  try {
    const res = await fetch(`${API_BASE_URL}/api/get-pause-status?device=${ACTIVE_DEVICE}`);
...
async function sendDiscordNotification(message, device = 'device1') {
  try {
    const res = await fetch(`${API_BASE_URL}/api/discord-notify`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
pauseBtn.addEventListener("click", async () => {
  ...
  try {
    showLoading(true);
    const res = await fetch(`${API_BASE_URL}/api/set-pause-status`, {
      method: "POST",
...
document.getElementById('delete-all-btn').addEventListener('click', async () => {
  ...
  try {
    showLoading(true);
    const res = await fetch(`${API_BASE_URL}/api/delete-all-data?device=${ACTIVE_DEVICE}`, { 
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json'
      }
